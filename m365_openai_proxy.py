...
- **Tool/function calling is EMULATED and probabilistic, not a guarantee.**
  ...
  **IMPORTANT if your client is OpenHands: prefer OpenHands' own
  ...
  than this proxy's own emulation has ever measured, under either design.
  ...
  "OpenHands' own client-side 'mock function calling'" section for the
  exact setup. Does not transfer to OpenCode (no equivalent client-side
  fallback exists) or to Goose (it DOES have an analogous mechanism,
  "Toolshim", but it didn't help in the prior design's testing -- see
  REVERSE_ENGINEERING.md's "Goose's own 'Toolshim'" section).
