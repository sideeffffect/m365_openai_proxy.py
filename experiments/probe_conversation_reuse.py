...
Answers the one genuinely-unconfirmed question blocking a Sydney-native
conversation mode (see REVERSE_ENGINEERING.md "Still open" / the
"goal re-scoped" section): if we open a Chathub WebSocket, send turn 1, close
it, then open a BRAND NEW WebSocket reusing the SAME `ConversationId` (new
`session_id`, new `X-SessionId`, new `clientrequestid`, exactly like a second,
independent `/v1/chat/completions` HTTP call would) and send a turn 2 that
only makes sense if the server remembers turn 1 -- does Sydney answer using
its own server-side memory, or does it behave like a fresh conversation?
...
def one_turn(auth, conversation_id, text, is_start_of_session):
    ...
    conversation_id instead of a fresh random one each call."""
